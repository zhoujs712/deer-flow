...
extensions_dir = Path(__file__).parent.parent / "extensions" / "chromadb" / "deerflow_chromadb"
sys.path.insert(0, str(extensions_dir.parent))

try:
    from deerflow_chromadb import ChromaMemoryStorage, IntentRecognitionTool
...
def example_business_data():
    ...
    print("\n2. Initializing intent recognition tool...")
    intent_tool = IntentRecognitionTool(storage)
    print("   ✓ Intent recognition tool initialized")
    
    # Step 1: Store business terms
    print("\n3. Storing business terms...")
    business_terms = [
        {
            "term": "用户留存率",
    ...
    for query in test_queries:
        ...
        if result.get('key_business_terms'):
            print(f"Business terms: {', '.join(result['key_business_terms'])}")
        
        # Show enhanced prompt (first part only)
